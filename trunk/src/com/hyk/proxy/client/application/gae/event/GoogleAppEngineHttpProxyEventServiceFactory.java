...
public class GoogleAppEngineHttpProxyEventServiceFactory implements HttpProxyEventServiceFactory
{
	...
	public void init(Config config, ExecutorService workerExecutor, StatusMonitor monitor) throws Exception
	{
		...
		{
			monitor.notifyStatus(fetchServices.size() + " fetch services are working.");
		}
		else
		{
	...
	protected List<FetchService> retriveFetchServices(final Config config) throws IOException, RpcException, XMPPException
	{
		...
		for(final HykProxyServerAuth auth : auths)
		{
			for(final RPC rpc : rpcs)
			{
				invokeTasks.add(new Callable<FetchService>()
				{
					...
					{
						return initFetchService(auth, rpc, config.getClient2ServerConnectionMode());
					}
				});
			}
		}
		...
		try
		{
			List<Future<FetchService>> invokeResults = workerExecutor.invokeAll(invokeTasks);
	...
	protected FetchService initFetchService(final HykProxyServerAuth appid, RPC rpc, ConnectionMode mode)
	{
		try
		{
			...
			switch(mode)
			{
				case HTTP2GAE:
				{
					remoteAddress = ClientUtils.createHttpServerAddress(appid.appid);
					break;
				}
				case XMPP2GAE:
				{
			...
			final RemoteServiceManager remoteServiceManager = rpc.getRemoteService(RemoteServiceManager.class, RemoteServiceManager.NAME,
					remoteAddress);
			AsyncRemoteServiceManager asyncRemoteServiceManager = RpcUtil.asyncWrapper(remoteServiceManager, AsyncRemoteServiceManager.class);
	...
	private void checkVersionCompatability(AsyncRemoteServiceManager remoteServiceManager, final String appid)
	{
		remoteServiceManager.getServerVersion(new RpcCallback<String>()
		{
			...
			public void callBack(RpcCallbackResult<String> result)
			{
				try
				{
					...
					if(Version.value.contains(serverVersion) || serverVersion.contains(Version.value))
					{
						return;
					}
	...
	static class FetchServiceSelector extends ListSelector<FetchService>
	{
		...
		{
			String binding = GoogleAppEngineApplicationConfig.getBindingAppId(req.getHeaderValue(HttpHeaders.Names.HOST));
			if(null != binding)
			{
		...
		public synchronized AsyncFetchService selectAsync(HttpRequestExchange req)
		{
