 ...
 */
package org.arch.compress.snappy;
...
import static org.arch.compress.snappy.SnappyInternalUtils.checkNotNull;
import static org.arch.compress.snappy.SnappyInternalUtils.checkPositionIndexes;
import static org.arch.compress.snappy.SnappyOutputStream.STREAM_HEADER;
import static org.arch.compress.snappy.SnappyOutputStream.MAX_BLOCK_SIZE;
...
public class SnappyInputStream extends InputStream
{
	...
	private void readInput(int length) throws IOException
	{
		...
		while (offset < length)
		{
			...
			{
				throw new EOFException(
				        "encountered EOF while reading block data");
			}
			offset += size;
		}
	}
		...
		do
		{
			...
			while (offset < header.length)
			{
				...
				if (size == -1)
				{
					...
					throw new EOFException(
					        "encountered EOF while reading block header");
				}
				offset += size;
			}
		}
		...
		switch (x)
		{
			...
			default:
				throw new IOException(format(
				        "invalid compressed flag in header: 0x%02x", x));
		}
	}
