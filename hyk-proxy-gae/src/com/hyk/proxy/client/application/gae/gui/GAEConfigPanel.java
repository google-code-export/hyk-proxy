...
public class GAEConfigPanel extends javax.swing.JPanel
{
	...
	private void initComponents()
	{
		...
		Collection<RegistCompressor> allCompressors = CompressorFactory
		        .getAllRegistCompressors();
		...
		Collection<RegistSecurityService> allSecServs = SecurityServiceFactory
		        .getAllRegistSecurityServices();
		...
		layout.setVerticalGroup(layout
		        ...
		        .addGroup(
		                ...
		                layout.createSequentialGroup()
		                        ...
		                        .addComponent(applyButton).addGap(19, 19, 19))
		        .addGroup(
		                layout.createParallelGroup(
		                        javax.swing.GroupLayout.Alignment.LEADING)
		                        .addGroup(
	...
	protected Config buildConfProps() throws Exception
	{
		...
		List<HykProxyServerAuth> list = new ArrayList<HykProxyServerAuth>(
		        appids.values());
		...
		config.setFetchLimitSize(Integer.parseInt(fetchLimitTextField1
		        .getText().trim()));
		config.setHttpConnectionPoolSize(Integer.parseInt(connectionPoolText
		        .getText().trim()));
		config.setCompressor(compressorType.getSelectedItem().toString());
		config.setMaxFetcherNumber(Integer.parseInt(fetcherNumText.getText()
		        .trim()));
	...
	private void isXMPPEnableActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_isXMPPEnableActionPerformed
		if (isXMPPEnable.isSelected())
		{
			enableXmpp();
		}
	...
	{// GEN-FIRST:event_deployButtonActionPerformed
		Misc.getGlobalThreadPool().submit(new Runnable()
		{
			...
			public void run()
			{
				...
				try
				{
					...
					{
						Future<Boolean> future = Misc.getGlobalThreadPool()
						        .submit(new Callable<Boolean>()
						        {
							        ...
							        public Boolean call() throws Exception
							        {
								        try
								        {
									        URL serverPackageUrl = new URL(
									                Constants.SERVER_PACKAGE_URL);
									        File deploy = new File(home,
									                "deploy");
									        File file = NetUtil.downloadFile(
									                serverPackageUrl, deploy);
									        if (null != file)
									        {
										        FileUtil.unzip(file, deploy,
										                true);
									        }
									        return Boolean.TRUE;
								        }
								        ...
								        {
									        logger.error(
									                "Failed to download gae server package!",
									                e);
								        }
								        return Boolean.FALSE;
							        }
						        });
						SwingHelper.showBusyButton(future, deployButton,
						        "Downloading...");
						try
						{
							if (!future.get())
							{
								...
								JOptionPane
								        .showMessageDialog(null,
								                "Faield to download server package, please refer log file for more details.");
								return;
							}
							else
							{
								SwingUtilities.getWindowAncestor(
								        GAEConfigPanel.this).dispose();
							}
						}
						catch (Exception e)
						{
					...
					Future future = Misc.getGlobalThreadPool().submit(
					        new Runnable()
					        {
						        ...
						        public void run()
						        {
							        try
							        {
								        String filesp = System
								                .getProperty("file.separator");
								        String appcfgwrapperjar = home
								                .getAbsolutePath()
								                + filesp
								                + "deploy"
								                + filesp
								        ...
								        cmd.append("java ");
								        String warPath = home.getAbsolutePath()
								                + filesp + "deploy" + filesp
								                + "war";
								        cmd.append("-Dappengine.app.location=")
								                .append(warPath).append(" ");
								        int row = appidTable.getSelectedRow();
								        if (row >= 0)
								        {
									        String appid = (String) appIdTableModel
									                .getValueAt(row, 0);
									        cmd.append("-Dappengine.appid=")
									                .append(appid).append(" ");
								        }
								        cmd.append(" -jar \"")
								                .append(appcfgwrapperjar)
								                .append("\"");
